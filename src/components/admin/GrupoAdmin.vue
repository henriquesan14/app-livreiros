<template>
  <div class="grupo-admin">
    <b-button
      ...
      class="mb-3"
      @click="zeraGrupo(); $bvModal.show('modal-grupo')"
      variant="dark"
    >
    ...
    <b-modal size="lg" id="modal-grupo" hide-footer>
      <template slot="modal-title">{{grupo.idGrupo == null ? 'Novo grupo' : 'Edição grupo'}}</template>
      <div class="d-block">
        <b-form @submit.prevent="submitGrupo()">
            ...
            <b-col md="12" sm="12">
              <div class="perm-group">
                ...
                >
                  ...
                  <button
                    :disabled="verificaPermissao(permissao.idPermissao)"
                    type="button"
                    @click="addPermissao(permissao)"
                    class="primary ml-1"
                  >
                    <i
                      class="fa"
                      :class="verificaPermissao(permissao.idPermissao) === true ? 'fa-ban':'fa-plus'"
                    ></i>
                  </button>
                </span>
              </div>
          ...
          <b-row>
            <b-col md="12" sm="12">
              <div class="perm-group">
                ...
                <h6
                  v-if="submitted && $v.grupo.permissoes.$invalid"
                  class="alert-perm"
                >Adicione pelo menos uma permissão</h6>

                <span class="mr-2 perms" v-for="perm in grupo.permissoes" :key="perm.idPermissao">
                  {{perm.nomePermissao}}
                  <button
                    type="button"
                    @click="removePermissao()"
                    class="ml-1 danger"
                  >
          ...
          <b-button class="mr-2" size="sm" type="submit" variant="success">
            <i class="fa fa-save mr-1"></i>Salvar
          </b-button>
          <b-button type="button" size="sm" @click="$bvModal.hide('modal-grupo')">
...
export default {
  ...
  methods: {
    ...
    verificaPermissao(idPermissao) {
      return this.grupo.permissoes.find(p => p.idPermissao === idPermissao)
        ? true
        : false;
    },
    submitGrupo() {
